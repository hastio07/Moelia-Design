    ...
    <script>
        ...
        /** $(document).ready(function() {
             ...
             albumProdukInput.on('change', function(e) {
                 ...
                 // Periksa jumlah saat ini apakah > max
                 ...
                 for (var i = 0; i < files.length; i++) {
                     ...
                     reader.onload = function(e) {
                         ...
                         var deleteButton = $('<button>').attr('type', 'submit').text('X').addClass('delete-button');
                         ...
                         let form = $('<form>').attr('action', '{{ route('home') }}').attr('method', 'POST');
                         form.append($('<input>').attr('type', 'hidden').attr('name', '_method').val('DELETE'));
                         form.append($('<input>').attr('type', 'hidden').attr('name', '_token').val('{{ csrf_token() }}'));
                         form.append(deleteButton);
                         var container = $('<div>').addClass('image-container').append(form, imgElement);
            ...
            if (window.File && window.FileReader && window.FileList && window.Blob) {
                ...
                if (files.length >= minPhotos && files.length <= maxPhotos) {
                    ...
                    if (files.length < minPhotos) {
                        alert('Batas foto yang diunggah adalah ' + minPhotos);
                        return false;
                    }
